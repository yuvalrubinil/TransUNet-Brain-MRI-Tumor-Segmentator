...
#### Loss Function:
Initially, the loss was set to be: $\mathcal{L} = 0.5 \cdot \mathcal{L}_{Dice} + 0.5 \cdot \mathcal{L}_{BCE}$. however, this setup performed poorly, the model consistently produced noisy, fragmented binary masks that looked like scattered "islands" rather than coherent tumor regions. To fix this, we shifted the focus and gave much more weight to the Dice score:
